...
def LSTM_Sentiment(input_tensor):
    ...
    output_rnn, _ = tf.compat.v1.nn.dynamic_rnn(
        lstmCell, input_tensor, dtype=tf.float32)
    ...
    output = tf.gather(output_transposed, int(
        output_transposed.get_shape()[0]) - 1)
...
def PixelRNN(inputs):
    ...
    output = conv2d(normalized_inputs, 16, [
                    7, 7], mask_type="a", scope="conv_inputs")
    ...
    for idx in range(2):
        output = tf.nn.relu(
            conv2d(output, 32, [1, 1], mask_type="b", scope='CONV_OUT%d' % idx))

    conv2d_out_logits = conv2d(
        output, 3, [1, 1], mask_type="b", scope='conv2d_out_logits')
